...
class Cadastro extends CI_Controller { // <--- CLASSE 'Cadastro'
    ...
    // A checagem de login no __construct() já garante que ninguém deslogado entre.
    
    $nivel_acesso = $this->session->userdata('nivel_acesso'); // Simplificando a variável
    
    // 1. Lógica de Restrição de Visualização:
    if ($nivel_acesso == 'admin') {
        // Se for ADMIN: Vê todos os registros
        $data['cadastros'] = $this->cadastro_model->get_all_cadastros();
        $data['titulo'] = 'Listagem de TODOS os Cadastros (ADMIN)'; // Título customizado
    } else {
        // Se for COMUM: Vê apenas os seus próprios registros
        $data['cadastros'] = $this->cadastro_model->get_cadastros_por_usuario();
        $data['titulo'] = 'Meus Cadastros (Usuário Comum)'; // Título customizado
    }
    
    // O resto da execução (carregar a View) permanece a mesma.
