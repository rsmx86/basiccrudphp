...
class Auth extends CI_Controller {
    ...
    // Método Padrão (Acessado via: http://localhost/ci3/index.php/auth)
    public function index() {
        // Se o usuário já estiver logado, redireciona para a página principal do CRUD (espero)
    ...
    public function login() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // Aqui chama o Model para verificar as credenciais
            $usuario = $this->usuario_model->verificar_login($email, $senha);

            if ($usuario) {
                // Se Sucesso =  Cria a sessão e armazena os dados importantes
                $session_data = array(
    ...
    // Cria um usuário admin inicial para testar
    // USO TEMPORARIO DE MODO (USO UNICO PARA CRIAR O ADMIN)
